...
begin
  ...
  if Trim(txt_num_doc.Text) = '' then
     begin
       ...
       Application.MessageBox('O campo Nº não poder vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
       Abort;
     end;
  ...
  if Trim(txt_valor.Text) = '' then
     begin
       txt_valor.SetFocus;
       Application.MessageBox('O campo valor não poder vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
       Abort;
     end;
